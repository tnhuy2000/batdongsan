...
class SYSNguoiDungController extends Controller
{
	...
	public function postCapNhatHoSo(Request $request)
	{
		$this->validate($request, [
			...
			'username_edit' => ['required', 'max:255', 'unique:sys_nguoidung,username,' . Auth::user()->id],
			'email_edit' => ['required', 'email', 'max:255', 'unique:sys_nguoidung,email,' . Auth::user()->id],
		]);
		
		$orm = SYS_NguoiDung::find(Auth::user()->id);
